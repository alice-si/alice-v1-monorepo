...
contract('Alternative tokens donations - single donation', function([owner, beneficiary, validator, donor, transformationService]) {
  var project;
  var mainToken;
  ...
  it("should transform an alternative donation step 2", async function() {
    await alternativeToken.burn(40, {from: transformationService});
  ...
  //     let expectedVal = 100 -(i+1)*10;
  //     (await project.getBalance(wallet.address)).should.be.bignumber.equal(expectedVal.toString());
  //   }
  // });
  //
  //
